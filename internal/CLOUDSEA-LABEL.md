...
| 字段 | 取值 | 是否进入当前云海 ML |
|------|------|---------------------|
| `status` | none / partial / full | ✅ 是（`train_cloudsea_model.py --approved-only`） |
| `sunrise_quality` | visible / blocked / unshootable | ❌ 否（先积累样本，中期单独训练日出模型） |

Admin **重训 ML** 仍只使用 `status`（有/无云海）。日出质量达标后计划新增 `train_sunrise_model.py` 或扩展训练脚本。
...
ML 训练需要 **30 个已标注日历日**（日出窗口 03:00–07:00 各标一次云海），含正负样本。

建议：有云海 10–15 天 + 无云海 15–20 天。日出质量可同步标注，便于日后训练。
